...
module.exports = {
    ...
    sendWhatsappMediaMessageURL: async function(requestQuery, frontendResponse, websocketConnection){
        ...
        exec(`curl -X  POST "https://graph.facebook.com/v17.0/100691516289394/messages" -H "Authorization: Bearer `+constants.credentials.apiKey+`" -H "Content-Type: application/json" -d "{messaging_product: 'whatsapp', recipient_type: 'individual', to: '`+requestQuery['recipientPhoneNumber']+`', type: '` + types[requestQuery['mediaType']] + `', ` + types[requestQuery['mediaType']] + ` : {link: '` +requestQuery['mediaURL']+`'}}"`, (error, stdout, stderr) => {
            if (error) {
    ...
    sendWhatsappMediaMessage: async function(requestQuery, frontendResponse, websocketConnection){
        ...
        exec(`curl -X POST "https://graph.facebook.com/v17.0/100691516289394/media" -H "Authorization: Bearer `+constants.credentials.apiKey+`" -F "file=@a` + extensions[requestQuery['mediaType']] + `" -F "type=` + requestQuery['mediaType'] + `" -F "messaging_product="whatsapp"`, (error, stdout, stderr) => {
            if (error) {
                console.log(`error: ${error.message}`);
                return;
            }
    ...
    sendAutomaticWhatsappMediaMessage: function(recipientPhoneNumber, mediaContent, messageContent, assignedAgentID, websocketConnection){
        ...
        exec(`curl -X POST "https://graph.facebook.com/v17.0/100691516289394/media" -H "Authorization: Bearer `+constants.credentials.apiKey+`" -F "file=@a.png" -F "type=image" -F "messaging_product="whatsapp"`, (error, stdout, stderr) => {
            if (error) {
                console.log(`error: ${error.message}`);
                return;
            }
            ...
            console.log(JSON.parse(stdout));
            exec(`curl -X  POST "https://graph.facebook.com/v17.0/100691516289394/messages" -H "Authorization: Bearer `+constants.credentials.apiKey+`" -H "Content-Type: application/json" -d "{messaging_product: 'whatsapp', recipient_type: 'individual', to: '`+recipientPhoneNumber+`', type: 'image', image : {id: ` +JSON.parse(stdout).id+`}}"`, (error, stdout, stderr) => {
                if (error) {
                    //console.log(`error: ${error.message}`);
                    return;
                }
